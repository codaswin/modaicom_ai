...
| | | No layout shift when the trigger appears | Pending | |
...
2. On a `legacy` post and an `sdui` feed post, the modaicom trigger beside the
   comment box is a small purple rounded-square button showing the **m** mark —
   **not** the word "modaicom".
3. Hover → it darkens and lifts slightly, and a "Generate with modaicom" tooltip
   ...
   clickable again.
4. The trigger does **not** push the comment box, the comment list, or anything
   else down or sideways when it appears (watch the layout as you scroll a fresh
   post into view).
5. Switch LinkedIn to Dark mode (Me → Settings → Account preferences → Dark
   mode). The trigger still looks right — solid purple, legible mark, visible
   focus ring.
6. LinkedIn's own comment actions (Comment / emoji / photo) are still clearly
