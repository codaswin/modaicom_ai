...
describe('session relay contract', () => {
  it('validates versioned extraction messages and rejects malformed payloads', () => {
    ...
    expect(isRelayMessage({ version: 2, type: 'INLINE_EXTRACTION_RESULT', generation: 2, sessionId: 'session-a', result })).toBe(true)
    expect(isRelayMessage({ version: 3, type: 'INLINE_EXTRACTION_RESULT', generation: 2, sessionId: 'session-a', result })).toBe(false)
    expect(isRelayMessage({ version: 2, type: 'INLINE_EXTRACTION_RESULT', generation: 2, sessionId: 'session-a', result: { kind: 'bad' } })).toBe(false)
    expect(isRelayMessage({ version: 2, type: 'UNKNOWN' })).toBe(false)
  })
