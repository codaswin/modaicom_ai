...
const COMMENT_ROOT_SELECTOR = '[data-testid*="comment" i], [data-test-id*="comment" i], [aria-label*="comment" i], [class*="comment" i]'
...
export function postCandidates(root: Document | Element = document, urlString = location.href): HTMLElement[] {
  ...
  return found.filter((candidate) => candidate.querySelector('[data-testid="post-body"], [data-testid="expandable-text-box"], [data-test-id="feed-shared-update-v2__description"], .feed-shared-update-v2__description, .feed-shared-inline-show-more-text'))
}
function commentRoot(editor: HTMLElement): HTMLElement | undefined {
  const root = editor.closest<HTMLElement>(COMMENT_ROOT_SELECTOR)
  if (!root) return undefined
  const marker = `${root.getAttribute('aria-label') ?? ''} ${root.className}`.toLowerCase()
  return /reply|message|search/.test(marker) ? undefined : root
}
export function composerIsEligible(editor: HTMLElement): boolean {
  if (!editor.isConnected || editor.closest(`[${OWNED_WRAPPER}]`) || !editor.matches(EDITOR_SELECTOR) || !commentRoot(editor)) return false
  ...
  return !/search|start a post|message|reply/.test(attrs)
}
...
function reconcile(): void { if (!isSupportedRoute()) { removeOwnedUi(); return } const seen = new Set<string>(); document.querySelectorAll<HTMLElement>(EDITOR_SELECTOR).forEach((editor) => { if (!composerIsEligible(editor)) return; const owner = owningPost(editor); if (!owner) return; const key = ownerKey(owner); if (seen.has(key)) return; seen.add(key); insertTrigger(editor, owner, key) }); document.querySelectorAll<HTMLElement>(`[${OWNED_WRAPPER}]`).forEach((node) => { if (!seen.has(node.dataset.modaicomOwner ?? '')) node.remove() }) }
export function observationScopes(urlString = location.href): HTMLElement[] { if (!isSupportedRoute(urlString)) return []; const main = document.querySelector<HTMLElement>('main'); const feed = feedRoot(); const posts = postCandidates(document, urlString); const scopes = isFeedRoute(urlString) ? (feed ? [feed] : main ? [main] : []) : (posts.length ? posts : main ? [main] : []); return Array.from(new Set(scopes)) }
function configureObserver(): void { observer?.disconnect(); if (!isSupportedRoute()) return; observer = new MutationObserver(scheduleReconcile); observationScopes().forEach((scope) => observer?.observe(scope, { childList: true, subtree: true })) }
function scheduleReconcile(): void { if (reconcileTimer !== undefined) window.clearTimeout(reconcileTimer); reconcileTimer = window.setTimeout(() => { reconcileTimer = undefined; if (location.href !== lastRoute) { lastRoute = location.href; removeOwnedUi(); sendClearRelay() } reconcile(); configureObserver() }, 50) }
...
export function teardownInlineTriggerContentScript(): void { observer?.disconnect(); observer = undefined; if (reconcileTimer !== undefined) { window.clearTimeout(reconcileTimer); reconcileTimer = undefined }; removeOwnedUi(); restoreHistoryHooks(); initialized = false }
